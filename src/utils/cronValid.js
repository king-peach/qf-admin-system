...
function checkListField(value, minimal, maximal, attribute) {
  ...
  for (var i = 0; i < values.length; i++) {
    // 判断枚举的值是否在范围内
    var message = checkIntValue(values[i], minimal, maximal, true, attribute)
    if (message !== true) {
      return message
    }
    count = 0
...
function checkIncrementField(value, minimal, maximal, attribute) {
  ...
  if (start !== '*') {
    // 检验前值是否正确
    var message = checkIntValue(start, minimal, maximal, true, attribute)
    if (message !== true) {
      return message
    }
    // 检验后值是否正确
...
function checkFieldWithLetter(value, letter, minimalBefore, maximalBefore, attribute) {
  ...
  if (letter === 'W') {
    if (!value.endsWith('W')) {
      ...
      num = value.substring(0, value.indexOf(letter))
      return checkIntValue(num, minimalBefore, maximalBefore, true, attribute)
    }
  }
...
function checkFieldWithLetterWeek(value, letter, minimalBefore, maximalBefore, attribute) {
  ...
  if (letter === '#') {
    ...
    var message = checkIntValue(num1, 1, 4, true, (attribute + '的#前面'))
    if (message !== true) {
      return message
    }
